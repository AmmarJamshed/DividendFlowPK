    ...
    <div className="space-y-6 pt-6 mt-6 border-t-2 border-slate-200">
      <div className="rounded-xl bg-gradient-to-br from-slate-50 via-white to-emerald-50/40 border border-slate-200 p-5 sm:p-6 shadow-sm">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-ice-600">Your portfolio projection</p>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            return (
              <button
                ...
                className={`group flex-1 min-w-0 flex flex-col items-center justify-end gap-1.5 h-full rounded-lg transition-colors ${
                  monthFilter === String(m.month) ? 'bg-slate-100 ring-2 ring-ice-500/40 ring-offset-1' : 'hover:bg-slate-50'
                }`}
                title={`${m.monthName}: ${formatRs(m.amount)}`}
              >
                {hasPay && (
                  <span className="text-[9px] sm:text-[10px] font-bold text-ice-600 opacity-0 group-hover:opacity-100 sm:opacity-100 transition-opacity truncate max-w-full px-0.5">
                ...
                <div
                  className={`w-full max-w-[2.5rem] mx-auto rounded-t-md transition-all ${
                    hasPay
                      ? isPeak
                        ? 'bg-gradient-to-t from-ink via-ice-600 to-emerald-500 shadow-md'
                        : 'bg-gradient-to-t from-ink-soft to-emerald-500'
                      : 'bg-slate-100'
                  }`}
                  style={{ height: `${pct}%`, minHeight: hasPay ? '8px' : '4px' }}
                />
                <span className={`text-[10px] sm:text-xs font-bold ${hasPay ? 'text-ink' : 'text-slate-400'}`}>
        ...
        {monthFilter !== 'all' && (
          <button
            ...
            className="mt-3 text-xs font-semibold text-ice-600 hover:underline"
          >
          ...
          {filteredRows.length > 8 && (
            <div className="px-4 py-3 border-t border-slate-100 bg-slate-50/50 text-center">
              <button
                ...
                className="text-sm font-semibold text-ice-600 hover:underline"
              >
...
export default function DividendCalculator({ symbolList = [] }) {
  ...
  return (
    ...
    >
      <div className="fin-hero fin-hero--brand px-5 sm:px-6 py-5 relative rounded-t-2xl">
      ...
      <div className="p-4 sm:p-6 space-y-5 bg-slate-50/50">
                    ...
                    <tr key={idx} className="border-b border-slate-50 last:border-0 hover:bg-slate-50">
                      ...
                      <td className="px-4 py-2">
                        <input
                          ...
                          placeholder="500"
                          className="w-full max-w-[140px] px-3 py-2.5 rounded-lg border border-slate-200 bg-slate-50/50 focus:bg-white focus:ring-2 focus:ring-ice-500/30 focus:border-ink focus:outline-none tabular-nums text-slate-800 placeholder:text-slate-400"
                        />
                      </td>
        ...
        {mode === 'pdf' && (
          <div className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6 shadow-sm space-y-4">
            ...
            >
              <span className="text-sm font-bold uppercase tracking-wide text-ice-600" aria-hidden>
                {pdfName ? 'Ready' : 'PDF'}
              </span>
            ...
            {loading && (
              <div className="flex items-center justify-center gap-3 text-sm text-ice-600">
                <div className="w-5 h-5 border-2 border-slate-200 border-t-ink rounded-full animate-spin" />
